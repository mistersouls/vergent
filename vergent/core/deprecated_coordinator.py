...
class CoordinatorDeprecated:
    ...
    def on_event(self, event: Event) -> None:
        ...
        if request_id and event.type in ("ok", "ko"):
            ...
            if event.type== "ok":
                pending.ack_success(payload["version"])
            else:
                pending.ack_failure()
    ...
    async def coordinate_delete(
        ...
        pending = Pending(quorum_write, self._loop)
    ...
    async def forward_put(
        ...
        # quorum_write is 1 because we forward
